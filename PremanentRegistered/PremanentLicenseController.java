...
public class PremanentLicenseController implements Initializable{
	@FXML Label validTill;
	@FXML Label date;
	@FXML Label name;
	@FXML Label address;
	@FXML Label dob;
	...
	public void setDetails(String name1) throws SQLException{
	  ...
	  public void takeSnapshot() throws IOException, SQLException{
		  Connection conection;
		  conection = SqliteConnection.connector();
		  PreparedStatement p = conection.prepareStatement("update Permanent set LicensePrinted = 1 where name = ?");
		  p.setString(1, name.getText());
		  p.executeUpdate();
			WritableImage image = license.snapshot(new SnapshotParameters(), null);
			File file = new File("C:\\java programs\\license"+name.getText()+".png");
